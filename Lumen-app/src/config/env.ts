 ...
 *
 * Was the Render deployment, which no longer exists — the whole platform moved
 * to a single Oracle instance and Render was deleted, so a build made from a
 * clone pointed at a dead host. Nothing secret lives here; it is the same
 * public URL the web console calls.
 *
 * Override it in .env to point at a local backend. */
const DEFAULT_API_URL = "https://140.238.250.246.sslip.io";
