...
export class AnalyticsController {
  ...
  @Get('trend')
  @ApiOperation({
    summary: 'Get daily complaint submission trend for the past N days',
  })
  @ApiQuery({
    name: 'days',
    required: false,
  ...
  getComplaintTrend(@Query('days') days?: string) {
    return this.analyticsService.getComplaintTrend(
  ...
  @Get('departments')
  @ApiOperation({
    summary: 'Get per-department resolution performance metrics',
  })
  ...
  @Get('recent-activity')
  @ApiOperation({
    summary: 'Get recent activity from audit logs and complaint timelines',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  @Get('sla')
  @ApiOperation({
    summary: 'Get SLA breach metrics for complaints over 48 hours',
  })
  getSlaMetrics() {
